...
function game(){
    ...
    afficheur.sendMessage("C'est au joueur "+joueurs[tour]+" de jouer");
    // Le nerf de la guerre 
    for(var i=0, len = pions.length; i < len; i++) {
        pions[i].addEventListener("click",function(){
            let sonClic = document.createElement("audio");
            sonClic.src = "../jeux/assets/sound/click.mp3";
            sonClic.play();
        ...
        if(!estValide(this)){
            ...
            afficheur.sendMessage("case occupée !!!<br /> Joueur "+joueurs[tour]+" Essaye une autre case.");
        } else{
            ...
            if(jeuEstFini){
                ...
                afficheur.sendMessage("Le joueur "+joueurs[tour]+" a Gagner !!! Bravo");
                let sonFini = document.createElement("audio");
                sonFini.src = "../jeux/assets/sound/victoire.mp3";
                sonFini.play();
                return;
            }
           ...
           if(matchNul(pions)) {
            // On affiche un message
            afficheur.sendMessage("Partie terminé ! Pas de gagnant cette fois-ci!");
            let sonPerdu = document.createElement("audio");
                sonPerdu.src = "../jeux/assets/sound/egalité.mp3";
                sonPerdu.play();
            return;
           } 
           ...
           tour++;
           tour = tour % 2;
           afficheur.sendMessage("Joueur "+joueurs[tour]+" à toi de jouer");
        }
        });
    }
